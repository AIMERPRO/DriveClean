...
def set_tmpval(id_user, state, intval=None, textval=None, is_protect=False):
...
def get_tmpval(id_user, state, is_delete_after_read=True):
...
def clear_user_tempvals(id_user):
